...
class Script(BaseScript):
    ...
    def bury(self):
        ...
        for i, item in enumerate(invent):
            if item == 'Bones' or item == 'Big bones':
...
##            mouse.move(mob_pos,(4,5),False,True)
        ...
        pos = game.locateNpc('closest')
        if pos != None:
            mouse.move(pos,(4,5),False,True)
...
##        self.timer.start()
        ...
        while not self._stop:
            if not self._pause:
                ...
                if 'Bones' in game.inventory.get(True) or 'Big bones' in game.inventory.get(True):
